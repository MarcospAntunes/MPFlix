...
function returnGenres({ movieGenreData, genre_ids }: returnGenresProps) {
  const genres = movieGenreData.map((genre) =>
    genre_ids.find((id: number) => genre.id == id)
  );
  const genresId: any[] = genres.filter((list: any) => list !== undefined);
  const genre = movieGenreData.filter(
    (genre: movie) => genre.id === genresId.find((id) => id == genre.id)
  );
